...
- **onnxruntime-web**: `scripts/fetch_ort.sh` downloads the pinned version from the npm registry, checks
  its sha512, and installs it in `app/lib/`. The container build and the desktop package scripts do this.
  `app/lib/` is not committed.
...
The same page also runs in a self-contained window, including VA2GKA Replay. The window's replay
source connects to `wss://didahsdr.guenael.ca/ws`. The page served by the local Python server still
uses `ws://localhost:9000/ws`. The package does not include that server or any recording.
...
On launch the window opens maximized on VA2GKA Replay. The other sources (KiwiSDR, sound card,
IC-7300, RTL-SDR) stay in the Source window.

The AppImage needs FUSE to run (`fuse2` on Arch: `sudo pacman -S fuse2`). If it quits with a message
about the Chrome sandbox or user namespaces, start it again with `--no-sandbox`.

A Windows zip can be built from Linux with `npm run dist:win`. That package is a folder you unzip and run; the
`.exe` keeps Electron's generic file icon. Installing Wine (`sudo pacman -S wine`) and turning
`signAndEditExecutable` back on in `desktop/package.json` stamps the didahSDR icon onto the `.exe`.
macOS packages are not built from here. `npm run dist:linux` and `npm run dist:win` run
`scripts/fetch_ort.sh` first, then pack `app/`, including `app/models/didahcw_v1_rc1.onnx` and the
onnxruntime files, so the CW decoder is in the binary.

Pushing a tag `v*` (for example `v0.8.3`) builds both packages and attaches them to that GitHub
Release. The workflow writes that version into `desktop/package.json` and into the two `v0.8.3`
strings in `app/index.html` (the page title and the top bar) before it packs the app. Nothing else
carries the version.
...
desktop/             Electron shell (see Desktop)
