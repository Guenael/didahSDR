 ...
 *
 * Expects demodulator.js then ic7300_if.js to have been addModule'd first
 * (designHalfband, ComplexFIR, RealIfConverter). The node is opened mono;
 * Chrome downmixes a stereo PCM2901. Packs ~25 ms of decimated Int16 I/Q.
 * No allocations in process() once the buffer pool is filled.
 */
        ...
        for (let i = 0; i < n; i++) {
            ...
            if (this.fill === this.framesTarget) {
                this.port.postMessage(
                    { type: 'iq', samples: this.cur, rate: this.outRate },
                    [this.cur.buffer]
                );
                this.cur = null;
                this.fill = 0;
